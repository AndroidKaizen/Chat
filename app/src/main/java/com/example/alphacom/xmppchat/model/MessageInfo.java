package com.example.alphacom.xmppchat.model;
...
public class MessageInfo implements Parcelable{
	...
	public static MessageInfo analyseMsgBody(String jsonStr) {
		...
		try {
			...
			String from = "";
			if (jsonObject.getString(MessageInfo.FROM).contains("/Smack")) {
				from = jsonObject.getString(MessageInfo.FROM).split("/Smack")[0];
			}
			msg.setFrom(from);
