...
import com.example.alphacom.xmppchat.adapter.ChatListAdapter;
import com.example.alphacom.xmppchat.customview.RecordButton;
...
public class ChatActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mXmppConnection = Const.connection;
        init();
    ...
    private void init() {
        ...
        // 获取Intent传过来的用户名
        this.pUSERID = getIntent().getStringExtra(EXTRA_USERID);
        this.userChat = getIntent().getStringExtra(EXTRA_FRIEND_NAME);/*
															 * + "/" +
															 * FriendListActivity
															 * .RESOUCE_NAME;
															 */
        userChatSendFile = userChat + "/" + FriendListActivity.MY_RESOUCE_NAME;
        this.pFRIENDID = getIntent().getStringExtra(EXTRA_FRIENDID);
        ...
        // 消息监听
        //cm = ChatManager.getInstanceFor(MyXmppConnection.getConnection());
        cm = ChatManager.getInstanceFor(mXmppConnection);
                    ...
                    public void processMessage(Chat chat2, Message message) {
                        ...
                        if (message.getFrom().contains(userChat)) {
                            ...
                            System.out.println("服务器发来的消息是 chat："
                                    + message.getBody());
                            //TODO remember
                            MessageInfo msgInfo = new MessageInfo(message.getTo(),message.getBody(),
                                    "", message.getFrom());
                            JSONObject msgJson = new JSONObject();
                            try {
                                msgJson.put("body",
                                        message.getBodies().iterator().next().getMessage().toString());
                                msgJson.put("from", message.getFrom());
                                msgJson.put("to", message.getTo());
                            ...
                            msg.obj = MessageInfo.toJson(msgInfo);
                            msg.sendToTarget();
    ...
    public void textMsg() {
        // 发送消息
        mSendButton = (Button) findViewById(R.id.formclient_btsend);
        // 发送消息给pc服务器的好友（获取自己的服务器，和好友）
        //TODO:应先检测是否有未读消息
        newchat = cm.createChat(userChat, new ChatMessageListener() {
...
//                .getInstanceFor(MyXmppConnection.getConnection());
        final FileTransferManager manager = FileTransferManager
    ...
    public void sendFile(String path, MessageInfo msg) {
         ...
         */
        // Create the file transfer manager
//        FileTransferManager sendFileManager = FileTransferManager
//                .getInstanceFor(MyXmppConnection.getConnection());
        FileTransferManager sendFileManager = FileTransferManager
                .getInstanceFor(mXmppConnection);
